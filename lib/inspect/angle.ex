defimpl Inspect, for: Angle do
  ...
  def inspect(%Angle{d: n}, opts) when is_number(n), do: concat ["#Angle<", to_doc(n, opts), @degrees, ">"]
  def inspect(%Angle{r: n}, opts) when is_number(n), do: concat ["#Angle<", to_doc(n, opts), @radians, ">"]
  def inspect(%Angle{g: n}, opts) when is_number(n), do: concat ["#Angle<", to_doc(n, opts), @gradians, ">"]

  def inspect(%Angle{dms: {d, 0, 0}}, opts) when is_integer(d), do: concat ["#Angle<", to_doc(d, opts), @degrees, ">"]
  ...
  def inspect(%Angle{dms: {d, m, s}}, opts) when is_integer(d) and is_integer(m) and is_number(s) do
    concat ["#Angle<",
            to_doc(d, opts), @degrees, " ",
            to_doc(m, opts), @prime, " ",
            to_doc(s, opts), @double_prime, ">"]
  end
end
